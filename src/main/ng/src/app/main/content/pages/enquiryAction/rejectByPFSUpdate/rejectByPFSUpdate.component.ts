...
@Component({
  ...
  templateUrl: './rejectByPFSUpdate.component.html',
  styleUrls: ['./rejectByPFSUpdate.component.scss']
})
export class RejectByPFSUpdateComponent {

    dialogTitle = "Rejected By PFS";
    ...
    submit(): void {
        if (this._rejectByForm.valid) {
            ...
            if (JSON.stringify(this._rejectBy) === JSON.stringify({})) { // Insert a new record ...
                ...
                this._enquiryActionService.createRejectByPFS(formValues).subscribe(response => {
                    this._matSnackBar.open('Reject by PFS details updated successfully.', 'OK', { duration: 7000 });
                ...
                this._enquiryActionService.updateRejectByPFS(this._rejectBy).subscribe(response => {
                    this._matSnackBar.open('Reject by PFS details updated successfully.', 'OK', { duration: 7000 });
                    this._dialogRef.close({ 'refresh': true, 'rejectBy': response });
                });
            }
